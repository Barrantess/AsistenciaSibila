...
void cpg_update_initialState(cpg_int idx, cpg_float val){
  cpg_params_vec[idx+0] = val;
  Canon_Outdated.h = 1;
}

void cpg_update_referenceState(cpg_int idx, cpg_float val){
...
void cpg_retrieve_prim(){
  CPG_Prim.controlAction[0] = ecos_workspace->x[2];
  CPG_Prim.controlAction[1] = ecos_workspace->x[3];
}
...
void cpg_solve(){
  ...
  if (!ecos_workspace ) {
    cpg_copy_all();
    ecos_workspace = ECOS_setup(4, 8, 0, 0, 2, (int *) &ecos_q, 0, Canon_Params_conditioning.G->x, Canon_Params_conditioning.G->p, Canon_Params_conditioning.G->i, 0, 0, 0, Canon_Params_conditioning.c, Canon_Params_conditioning.h, 0);
  } else {
    if (Canon_Outdated.h) {
      cpg_copy_h();
        for (i=0; i<8; i++) { ecos_updateDataEntry_h(ecos_workspace, i, Canon_Params_conditioning.h[i]); };
    }
  }
